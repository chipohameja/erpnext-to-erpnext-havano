...
class ERPNextToERPNextHavanoAPI:
	...
	def sync_data(self, doctype):
		...
		try:
			self.items = requests.get(f'{self.cloud_url}{doctype}?fields=["*"]', headers=self.headers).json()
				...
				if item["custom_synced"] == 0:
					if not frappe.db.exists(doctype, item["name"]):
						...
						put_response = requests.post(
							f"{self.cloud_url}api/method/erpnext_to_erpnext_havano.api.update_item",
