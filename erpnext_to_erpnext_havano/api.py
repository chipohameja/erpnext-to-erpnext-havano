...
def sync_data(doctype):
    ...
    try:
        items_response = make_request(
            session,
            "get",
            f'{cloud_url}/api/resource/{doctype}?filters=[["custom_synced", "=", "0"]]&fields=["*"]'
        )
            ...
            try:
                if item.get("custom_synced") == 0 and not frappe.db.exists(doctype, item["name"]):
                    ...
                    try:
                        ...
                        frappe.db.commit()
                        frappe.log_error(
                            title=f"Sync time: {item['name']} in {doctype}",
...
def sync_doctypes():
    if cloud_url and local_url and sync_settings.is_local == 1:
        doctypes = [
            "Role", "User", "Company", "Account", "Customer",
        ...
        try:
            ...
            for inv_name in invoices:
                try:
                    ...
                    frappe.log_error(title="Invoice Prepare Error", message=str(inv_err))
            if processed_inv:
                res = make_request(
                    session,
                    "post",
                ...
                if res.status_code == 200:
                    ...
                    for inv in processed_inv_data:
                        ...
                        frappe.log_error(
                            title="Sales Invoice Synced",
                            message=f"Sales Invoice {inv_name} marked as synced with reference: {reference}, on cloud at {invoice_time}."
                        )
                    frappe.db.commit()
...
def update_item(docs):
    for doc in docs:
        ...
        frappe.log_error(
            title="Sync Update",
            message=f"{doctype} '{name}' marked as synced at {sync_time}."
        )
...
def update_invoice(docs):
    ...
    for data in docs:
        try:
            ...
            frappe.db.commit()
            frappe.log_error(
                title=f"{new_sale.name} synced successfully from local",
                message=f"{new_sale.name} synced successfully from local with {data.get('name')} at {sales_invoice_insert_time}."
            )
            ...
            frappe.log_error(
                title="Invoice Insert Failed",
                message=f"{data.get('reference_invoice')}: {str(e)}"
            )
            raise e
...
def send_email(recipient, subject, message):
    try:
        ...
        frappe.log_error(title="Email Send Failed", message=str(e))
